...
static std::map<RtpExtType/*id*/, std::string/*ext*/> s_type_to_url = {RTP_EXT_MAP(XX)};
#undef XX


#define XX(type, url) {url, RtpExtType::type},
static std::unordered_map<std::string/*ext*/, RtpExtType/*id*/> s_url_to_type = {RTP_EXT_MAP(XX)};
#undef XX

RtpExtType RtpExt::getExtType(const std::string &url) {
...
const std::string& RtpExt::getExtUrl(RtpExtType type) {
    ...
    if (it == s_type_to_url.end()) {
        throw std::invalid_argument(std::string("未识别的rtp ext类型:") + std::to_string((int) type));
    }
    return it->second;
}
...
//   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
std::string RtpExt::getSdesMid() const {
...
std::string RtpExt::getRtpStreamId() const {
    //CHECK(_type == RtpExtType::sdes_rtp_stream_id && size() >= 1);
    return *this;
}

std::string RtpExt::getRepairedRtpStreamId() const {
...
std::string RtpExtContext::getRid(uint32_t ssrc) const{
...
void RtpExtContext::setRid(uint32_t ssrc, const std::string &rid) {
    _ssrc_to_rid[ssrc] = rid;
}

RtpExt RtpExtContext::changeRtpExtId(const RtpHeader *header, bool is_recv, std::string *rid_ptr, RtpExtType type) {
    std::string rid, repaired_rid;
    RtpExt ret;
...
void RtpExtContext::onGetRtp(uint8_t pt, uint32_t ssrc, const std::string &rid){
    if (_cb) {
        _cb(pt, ssrc, rid);
    }
}
