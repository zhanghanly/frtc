...
std::string FCI_SLI::dumpString() const {
    std::stringstream ss;
    ss << "First:" << getFirst() 
       << ", Number:" << getNumber() 
       << ", PictureID:" << (int)getPicID();
...
std::string FCI_FIR::dumpString() const {
    std::stringstream ss;
    ss << "ssrc:" << getSSRC() 
       << ", seq_number:" << (int)getSeq() 
...
std::string FCI_REMB::create(const std::vector<uint32_t> &ssrcs, uint32_t bitrate) {
...
std::vector<uint32_t> FCI_REMB::getSSRC() {
...
std::string FCI_REMB::dumpString() const {
    //_StrPrinter printer;
    std::stringstream ss;
    ss << "bitrate:" << getBitRate() << ", ssrc:";
...
std::vector<bool> FCI_NACK::getBitArray() const {
    std::vector<bool> ret;
...
std::string FCI_NACK::dumpString() const {
...
std::string RunLengthChunk::dumpString() const {
    std::stringstream printer;
...
public:
    ...
    // 获取symbollist
    std::vector<SymbolStatus> getSymbolList() const;
    // 构造函数
    StatusVecChunk(bool symbol_bit, const std::vector<SymbolStatus> &status);
    // 打印本对象
    std::string dumpString() const;
};
#pragma pack(pop)

StatusVecChunk::StatusVecChunk(bool symbol_bit, const std::vector<SymbolStatus> &status) {
...
std::vector<SymbolStatus> StatusVecChunk::getSymbolList() const {
    //CHECK(type == 1);
    std::vector<SymbolStatus> ret;
...
std::string StatusVecChunk::dumpString() const {
    std::stringstream printer;
...
std::string FCI_TWCC::dumpString(size_t total_size) const {
    std::stringstream printer;
...
static void appendDeltaString(std::string &delta_str, FCI_TWCC::TwccPacketStatus &status, int count) {
...
std::string FCI_TWCC::create(uint32_t ref_time, uint8_t fb_pkt_count, TwccPacketStatus &status) {
    std::string fci;
