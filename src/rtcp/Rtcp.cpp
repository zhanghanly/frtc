...
static void setupHeader(RtcpHeader* rtcp, RtcpType type, size_t report_count, size_t total_bytes) {
    ...
    if (report_count > 0x1F) {
        ...
        throw std::invalid_argument("rtcp report_count最大赋值为31,当前为:");
    }
    // items总个数
...
std::string RtcpHeader::dumpHeader() const {
...
std::string RtcpHeader::dumpString() const {
    switch ((RtcpType)pt) {
    ...
    switch ((RtcpType)pt) {
        ...
        case RtcpType::RTCP_XR: {
            ...
            if (xr->bt == 4) {
                ...
                throw std::runtime_error("rtcp xr bt  not support");
            }
            break;
        }
        //default: throw std::runtime_error(StrPrinter << "未处理的rtcp包:" << rtcpTypeToStr((RtcpType)this->pt));
        default: throw std::runtime_error("未处理的rtcp包:");
    }
}
...
std::vector<RtcpHeader*> RtcpHeader::loadFromBytes(char* data, size_t len) {
    ...
    while (remain > (ssize_t)sizeof(RtcpHeader)) {
        ...
        if (remain < (ssize_t)rtcp_len) {
            //std::cout << "非法的rtcp包,声明的长度超过实际数据长度";
            break;
        }
        try {
...
std::string RtcpSR::dumpString() const {
    std::stringstream printer;
...
std::string ReportItem::dumpString() const {
    std::stringstream printer;
    printer << "ssrc:" << ssrc << "\r\n";
...
std::string RtcpRR::dumpString() const {
    std::stringstream printer;
    printer << RtcpHeader::dumpHeader();
    printer << "ssrc:" << ssrc << "\r\n";
...
std::string SdesChunk::dumpString() const {
    std::stringstream printer;
    printer << "ssrc:" << ssrc << "\r\n";
    printer << "type:" << sdesTypeToStr((SdesType)type) << "\r\n";
    printer << "txt_len:" << (int)txt_len << "\r\n";
    printer << "text:" << (txt_len ? std::string(text, txt_len) : "") << "\r\n";
...
std::string RtcpSdes::dumpString() const {
    std::stringstream printer;
    printer << RtcpHeader::dumpHeader();
...
std::string RtcpFB::dumpString() const {
    std::stringstream printer;
    printer << RtcpHeader::dumpHeader();
    printer << "ssrc:" << ssrc << "\r\n";
...
std::string RtcpBye::getReason() const {
    ...
    return std::string((char *)reason_len_ptr + 1, *reason_len_ptr);
}

std::string RtcpBye::dumpString() const {
    std::stringstream printer;
    printer << RtcpHeader::dumpHeader();
...
////////////////////////////////////////////
std::string RtcpXRRRTR::dumpString() const {
    std::stringstream printer;
    printer << RtcpHeader::dumpHeader();
    printer << "ssrc :" << ssrc << "\r\n";
    printer << "bt :" << (int)bt << "\r\n";
...
void RtcpXRRRTR::net2Host(size_t size) {
    ...
    if (size != sizeof(RtcpXRRRTR)) {
        ...
        //               << " actual size " << size);
        throw std::invalid_argument("rtcp xr Receiver Reference Time Report Block must is");
    }
    ssrc = ntohl(ssrc);
...
std::string RtcpXRDLRRReportItem::dumpString() const {
    std::stringstream printer;
    printer << "ssrc :" << ssrc << "\r\n";
...
std::string RtcpXRDLRR::dumpString() const {
    std::stringstream printer;
    printer << RtcpHeader::dumpHeader();
    printer << "ssrc :" << ssrc << "\r\n";
    printer << "bt :" << (int)bt << "\r\n";
