...
DepLibSRTP::DepLibSRTP() {
    ...
    LOG_DEBUG("libsrtp version: %s", srtp_get_version_string());
    ...
    if (DepLibSRTP::IsError(err)) {
        ...
        LOG_ERROR("srtp_init() failed: %s", DepLibSRTP::GetErrorString(err));
    }
    ...
    err = srtp_install_event_handler([](srtp_event_data_t *data) {
        ...
        switch (data->event) {
            ...
            case event_ssrc_collision: LOG_WARN("%s", "SSRC collision occurred"); break;
            //case event_key_soft_limit: MS_WARN_TAG(srtp, "stream reached the soft key usage limit and will expire soon"); break;
            case event_key_soft_limit: LOG_WARN("%s", "stream reached the soft key usage limit and will expire soon"); break;
            //case event_key_hard_limit: MS_WARN_TAG(srtp, "stream reached the hard key usage limit and has expired"); break;
            case event_key_hard_limit: LOG_WARN("%s", "stream reached the hard key usage limit and has expired"); break;
            //case event_packet_index_limit: MS_WARN_TAG(srtp, "stream reached the hard packet limit (2^48 packets)"); break;
            case event_packet_index_limit: LOG_WARN("%s", "stream reached the hard packet limit (2^48 packets)"); break;
        }
    });

    if (DepLibSRTP::IsError(err)) {
        //MS_THROW_ERROR("srtp_install_event_handler() failed: %s", DepLibSRTP::GetErrorString(err));
        LOG_ERROR("srtp_install_event_handler() failed: %s", DepLibSRTP::GetErrorString(err));
    }
}
...
SrtpSession::SrtpSession(Type type, CryptoSuite cryptoSuite, uint8_t *key, size_t keyLen) {
    ...
    default: {
        ...
        LOG_ERROR("%s", "unknown SRTP crypto suite");
    }
    }
    ...
    if (DepLibSRTP::IsError(err))
        ...
        LOG_ERROR("srtp_create() failed: %s", DepLibSRTP::GetErrorString(err));
}
...
SrtpSession::~SrtpSession() {
    ...
    if (this->session != nullptr) {
        ...
        if (DepLibSRTP::IsError(err))
            ...
            LOG_ERROR("srtp_dealloc() failed: %s", DepLibSRTP::GetErrorString(err));
    }
}
...
bool SrtpSession::EncryptRtp(uint8_t *data, int *len) {
    ...
    if (DepLibSRTP::IsError(err)) {
        LOG_WARN("srtp_protect() failed:%s", DepLibSRTP::GetErrorString(err));
        return false;
    }
...
bool SrtpSession::DecryptSrtp(uint8_t *data, int *len) {
    ...
    if (DepLibSRTP::IsError(err)) {
        LOG_WARN("srtp_unprotect() failed:%s", DepLibSRTP::GetErrorString(err));
        return false;
    }
...
bool SrtpSession::EncryptRtcp(uint8_t *data, int *len) {
    ...
    if (DepLibSRTP::IsError(err)) {
        LOG_WARN("srtp_protect_rtcp() failed:%s", DepLibSRTP::GetErrorString(err));
        return false;
    }
...
bool SrtpSession::DecryptSrtcp(uint8_t *data, int *len) {
    ...
    if (DepLibSRTP::IsError(err)) {
        LOG_WARN("srtp_unprotect_rtcp() failed:%s", DepLibSRTP::GetErrorString(err));
        return false;
    }
